...
class EbookController extends Controller
{
    ...
    public function store(Course $course, Author $author, EbookUploadRequest $request): JsonResponse
    {
    ...
    {
        $ebooks = Ebook::with(['author', 'course.category'])
            ->where('id', '!=', 5)
            ->paginate(10);return response()->json(EbookResource::collection($ebooks));
    }
    ...
    public function getSupplementalMaterial(Course $course): JsonResponse
    {
        $supplementalEbook = Ebook::where('course_id', $course->id)
            ->where('title', 'Vodič - od ideje do realizacije')
            ->first();

        if (! $supplementalEbook) {
            return response()->json(['message' => 'Dodatni materijal nije pronađen za ovaj kurs.'], 404);
        }

        $url = Storage::disk('r2')->temporaryUrl($supplementalEbook->path, now()->addMinutes(30));

        return response()->json([
            'pdf' => [
                'url'   => $url,
                'title' => $supplementalEbook->title,
                'thumbnail' => $supplementalEbook->thumbnail
            ],
        ]);
    }
}
